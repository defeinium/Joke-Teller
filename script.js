...
function tellMe(joke) {
    VoiceRSS.speech({
        key: '5e4e7d9478d14e64a949d03ede5a7123',
        src: joke,
        hl: 'en-us',
        v: 'Linda',
        r: 0, 
...
async function getJokes() {
    let joke = ''; //empty string
    const apiUrl = 'https://sv443.net/jokeapi/v2/joke/Programming?blacklistFlags=nsfw,religious,political,racist,sexist';
    try {
        const response = await fetch(apiUrl);
        const data = await response.json(); //turn response into json
        if (data.setup) {
            joke = `${data.setup} ... ${data.delivery}`; // to add string andvar into one ``
        } else {
            joke = data.joke;
        }
        tellMe(joke);
